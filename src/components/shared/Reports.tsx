...
import { mockIssues } from '../../services/mock';
...
export default function Reports() {
  const handleExportCSV = () => {
    try {
      exportToCSV(mockIssues, 'citiscope-full-export');
  ...
  const handleExportPDF = () => {
    try {
      ...
      exportToPDF(mockIssues, columns, 'citiscope-full-report', 'CitiScope Full Infrastructure Report');
  ...
  const handleExportJSON = () => {
    try {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(mockIssues, null, 2));
      const downloadAnchorNode = document.createElement('a');
      downloadAnchorNode.setAttribute("href",     dataStr);
      downloadAnchorNode.setAttribute("download", "citiscope-data.json");
  ...
  return (
    <div className="space-y-6">
      <Meta 
        title="Reports & Exports | CitiScope" 
        description="Generate and export detailed system data for offline analysis."
      />
      <div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Button variant="outline" className="flex flex-col h-auto py-4 gap-2" onClick={handleExportCSV}>
                <FileSpreadsheet className="h-5 w-5 text-green-600" />
                <span>Export CSV</span>
              </Button>
              <Button variant="outline" className="flex flex-col h-auto py-4 gap-2" onClick={handleExportPDF}>
                <FileText className="h-5 w-5 text-red-600" />
                <span>Export PDF</span>
              </Button>
              <Button variant="outline" className="flex flex-col h-auto py-4 gap-2" onClick={handleExportJSON}>
