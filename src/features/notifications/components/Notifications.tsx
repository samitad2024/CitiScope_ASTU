...
export default function Notifications() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Meta 
        title="Notifications | CitiScope" 
        description="Stay updated with real-time alerts and system events."
      />
                ...
                <div key={n.id} className={`p-4 flex gap-4 transition-colors hover:bg-muted/50 ${!n.read ? 'bg-primary/5' : ''}`}>
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      {!n.read && <Button variant="ghost" size="sm" className="h-7 text-[10px]" onClick={() => markAsRead(n.id)}>Mark as read</Button>}
                      <Button variant="ghost" size="sm" className="h-7 text-[10px] text-destructive" onClick={() => removeNotification(n.id)}>Delete</Button>
                    </div>
                  </div>
                </div>
              ))}
